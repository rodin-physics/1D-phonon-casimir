...
for ii = 1 : length(Ds)
    D = Ds[ii];
    E0 = Energy_Integral(D, α, T_min)
    println(ii)
    r =  map(x -> Energy_Integral(D, α, x) / E0, Ts);
    plot!(Ts, (r), linewidth = 2,  color = colors[ii], lab = latexstring(L"$D = $" * string(D)))
end

plot!(xs, ys, linewidth = 2, color = colors[5], line = :dash, lab = :false)
